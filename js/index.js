    ...
    sreach.prototype = {
        ...
        createSreachListHTML: function (keywolds) {
            ...
            for (var i = 0; i < arr.length; i++) {
                ...
                if (self.isSreachIndexOF(arr[i].name, keywolds) || self.isSreachIndexOF(arr[i].des, keywolds)) {
                    var myLi = document.createElement("LI");
        ...
        valToHTML: function (val) {
            var self = this;
            val ? (self.isTagSearch(val) ? self.createTagsListHTML(val) : self.createSreachListHTML(val)) : self.creatListHTML();
            if (window.history && window.history.pushState) val ? history.pushState({}, "fedocker", "?kw=" + val) : history.pushState({}, "fedocker", "/");
